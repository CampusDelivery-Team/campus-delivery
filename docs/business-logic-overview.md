...
本文既是业务总览，也是后续开发必须遵守的业务规则。当前数据库表结构已经部署到服务器，后续开发以现有表关系为基础，不通过修改表结构来改变主流程。
...
- 不在任务接派前创建支付记录，也不采用“先支付、后接单”的预付流程。
- 不把任务提前写成 `FINISHED`；只有送达确认和付款规则满足后才能完成。
- 一个任务只能写入一种任务明细，不得同时写入外卖、快递和私人跑腿明细。
- 不物理删除已经进入业务链路的任务、接派、支付、退款、评价、投诉、结算和审计历史。
- 不删除已经被历史任务引用的用户、跑腿员、节点、服务类型和服务节点规则。
- 结算明细只能关联属于对应跑腿员、已支付、已完成且无活动争议的支付记录。
- 所有跨表写入必须处于同一事务；竞争写操作按需要锁定业务行。
- Repository 只处理 SQL 和原始数据，Service 负责权限、状态机、事务和业务规则。
...
- 三类基础数据价格分别为外卖分发3元、快递代取4元、私人跑腿5元；实际基础价以 `service_types.base_price` 为准，可由管理员维护。
...
- 评价写入、编辑或删除与信誉分调整使用同一事务，信誉分始终限制在0至100，`credit_delta` 保存考虑上下限后实际生效的变化值。
