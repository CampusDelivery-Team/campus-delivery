...
- 当前未发现 `Views/Complaint` 页面目录，投诉模块页面可能尚未完成。
- 投诉处理状态目前为 `SUBMITTED`、`PROCESSING`、`DONE`，没有明确“成立/驳回”字段。
- 投诉处理后是否扣减跑腿员信誉分，目前从代码看尚不完整。
...
2. 退款申请通过但未完成时，支付状态是否会立即变为 REFUNDED？
...
| 风险 | 影响 | 处理方式 |
| --- | --- | --- |
| 组员8投诉页面未完成 | 结算无法完整验证投诉阻断 | 第一版按 `complaints` 表状态查询；页面由组员8补齐 |
